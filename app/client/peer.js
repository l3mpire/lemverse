...
peer = {
  ...
  video(enabled, notifyNearUsers = false) {
    ...
    if (enabled && notifyNearUsers) userProximitySensor.callProximityStartedForAllNearUsers();
    if (!enable) document.querySelector('.js-video-me').style.backgroundImage = `url(https://robohash.org/${encodeURI(Meteor.user().profile.name)}?set=set4&bgset=bg2&size=164x124)`;
